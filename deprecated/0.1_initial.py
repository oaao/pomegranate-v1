...
def market_import():
    ...
    print('\nMARKET_IMPORT: Trying: ' + url_market)
    data_items = []
...
def market_distill(raw_list, configs):
    ...
    sort_options = list(data_timestamp[0].keys())
    sort_choice = ''
    while sort_choice not in sort_options:
        sort_choice = input('MARKET_DISTILL: Sort by %s ?\n' % sort_options).lower()
    t_typesort = time.time()
    data_typesort = group_dictvalue(data_timestamp, sort_choice)
    print('\nMARKET_DISTILL: %s total entries grouped by %s items.'
...
with open("structured_json.txt", "w") as f_output:
